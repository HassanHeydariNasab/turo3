...
func _ready():
	...
	Senfinfino.set_global_position(Vector2(0,-101300))  #-- NOTE: Automatically converted by Godot 2 to 3 converter, please review
	...
	for i in range(-101000, -500, 700):
		...
		Rektangulo_.set_global_position(  #-- NOTE: Automatically converted by Godot 2 to 3 converter, please review
	...
	for i in range(-101000, 0, 250):
		...
		Materialo_.set_global_position(  #-- NOTE: Automatically converted by Godot 2 to 3 converter, please review
	...
	for i in range(-101000, -4000, 12000):
		...
		Ebeno_.set_global_position(  #-- NOTE: Automatically converted by Godot 2 to 3 converter, please review
			Vector2(300, i+rand_range(-2000,4000))
		)
		Ebenoj.add_child(Ebeno_)
	Fonmuziko.set("playing", T.Agordejo.get_value("Agordoj", "Muzikoj", true))
...
func _input(evento):
	if evento is InputEventMouseButton:  #-- NOTE: Automatically converted by Godot 2 to 3 converter, please review
		if evento.button_index == BUTTON_LEFT:
			if evento.position.y > 70 and evento.position.x < 570:  #-- NOTE: Automatically converted by Godot 2 to 3 converter, please review
				if evento.is_pressed() and materialo > 0:
					konsumita_materialo = 0
					PreParto.set_scale(Vector2(1,1))
					PreParto.set_global_position(get_global_mouse_position())  #-- NOTE: Automatically converted by Godot 2 to 3 converter, please review
					PreParto.show()
					Kreski_sono.stop()
					Kreski_sono.set("playing", T.Agordejo.get_value("Agordoj", "Sonoj", true))
					...
					if enajxoj.size() == 0:
						...
						if oktavo == 3:
							if alto < 2000:
								C3_spiccato.set("playing", T.Agordejo.get_value("Agordoj", "Sonoj", true))
							elif alto > 8000:
								B3_spiccato.set("playing", T.Agordejo.get_value("Agordoj", "Sonoj", true))
							elif alto > 4000:
								G3_spiccato.set("playing", T.Agordejo.get_value("Agordoj", "Sonoj", true))
							elif alto >= 2000:
								E3_spiccato.set("playing", T.Agordejo.get_value("Agordoj", "Sonoj", true))
							oktavo = 5
						elif oktavo == 5:
							if alto < 2000:
								C5_spiccato.set("playing", T.Agordejo.get_value("Agordoj", "Sonoj", true))
							elif alto > 8000:
								B5_spiccato.set("playing", T.Agordejo.get_value("Agordoj", "Sonoj", true))
							elif alto > 4000:
								G5_spiccato.set("playing", T.Agordejo.get_value("Agordoj", "Sonoj", true))
							elif alto >= 2000:
								E5_spiccato.set("playing", T.Agordejo.get_value("Agordoj", "Sonoj", true))
							oktavo = 3
						var Parto_ = Parto.instance()
						Parto_.set_global_position(PreParto.get_global_position())  #-- NOTE: Automatically converted by Godot 2 to 3 converter, please review
						var Formo_ = RectangleShape2D.new()
						Formo_.set_extents(Vector2(10,10)*PreParto.get_scale())
						Parto_.create_shape_owner(Formo_)
						Parto_.shape_owner_add_shape(0, Formo_)
			...
			V_rulumilo.set_value(V_rulumilo.get_value()+100)
	elif evento is InputEventMouseMotion:  #-- NOTE: Automatically converted by Godot 2 to 3 converter, please review
		if evento.position.y > 70 and evento.position.x < 570:  #-- NOTE: Automatically converted by Godot 2 to 3 converter, please review
			PreParto.set_global_position(get_global_mouse_position())  #-- NOTE: Automatically converted by Godot 2 to 3 converter, please review
...
func _on_H_rulumilo_value_changed( valoro ):
	...
	Fono.set_global_position(Kamero.get_offset()+Vector2(-300,-500))  #-- NOTE: Automatically converted by Godot 2 to 3 converter, please review
...
func _on_T500_timeout():
	...
	for Parto_ in Partoj.get_children():
		if Parto_.get_collision_layer_bit(0):
	...
	if diferenco > 2:
		if not E3_timpani.is_playing():
			E3_timpani.set("playing", T.Agordejo.get_value("Agordoj", "Sonoj", true))
		if averagxa_rapido_x > 6:
			E3_tremolo.set_volume_db(-15)
			G3_tremolo.set_volume_db(-3)
			if not G3_tremolo.is_playing():
				G3_tremolo.set("playing", T.Agordejo.get_value("Agordoj", "Sonoj", true))
		elif averagxa_rapido_x > 2:
			E3_tremolo.set_volume_db(-3)
			G3_tremolo.set_volume_db(-15)
			if not E3_tremolo.is_playing():
				E3_tremolo.set("playing", T.Agordejo.get_value("Agordoj", "Sonoj", true))
	elif partoj >= 20:
		if diferenco < -50:
			C3_chimes.set("playing", T.Agordejo.get_value("Agordoj", "Sonoj", true))
			T30000.start()
			T500.stop()
	elif partoj < 20 and partoj > 5:
		if diferenco < -25:
			C4_chimes.set("playing", T.Agordejo.get_value("Agordoj", "Sonoj", true))
			T30000.start()
			T500.stop()
...
func _on_PreParto_body_entered( korpo ):
	enajxoj.append(korpo)

func _on_PreParto_body_exited( korpo ):
	enajxoj.erase(korpo)
